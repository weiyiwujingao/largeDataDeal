 ...
 *---------------------------------------------------------------
 * $author:linfeng $addtime:2017-05-23
 ****************************************************************/
	...
	function get_tops($limit = 10, $xattr = '')
	{
		...
		while ($tmpa = $this->get_result())
		{
			foreach ($tmpa as $tmp)
			{
				...
				if (isset($list[$word]))
				{
					$list[$word]['weight'] += formats($tmp['idf']);
					$list[$word]['total']++;
				}
				else
				{
					$list[$word] = array('word'=>$tmp['word'], 'total'=>1, 'weight'=>formats($tmp['idf']), 'attr'=>$tmp['attr']);
				}
			}
		}
