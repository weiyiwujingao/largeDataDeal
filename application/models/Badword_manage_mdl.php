...
class Badword_manage_mdl extends MY_Controller
{
	/* 敏感词表 */
   ...
   /**
	...
	{
		if(isset($where['word']) && !empty($where['word']))
			$this->db->where('word', trim($where['word']));
		...
		$query = $this->db->select('id,word,type,appid,state,createuser,uptime')
						  ->from(self::TBL_BADWORD)
						  ->order_by('id desc')
   ...
   /**
     * 添加敏感词
     * 
	 * @param string  $data['word']  敏感词
	 * @param integer $data['type']  敏感词类型
	 * @param integer $data['state'] 敏感词状态
     * @return integer
     */
	public function addWord($data)
	{
        $this->db->insert(self::TBL_BADWORD, $data);
   ...
   /**
     ...
     * @return integer
     */
	public function editWord($id, $data)
	{
        $this->db->update(self::TBL_BADWORD, $data, array('id' => $id));
//t($this->db->affected_rows());
   ...
   /**
	...
	{
        $this->db->delete(self::TBL_BADWORD, array('id' => $id));

		return ($this->db->affected_rows() == 1) ? TRUE : FALSE;
	}
}

/* End of file Badword_manage_mdl.php */
/* Location: ./application/controllers/Badword_manage_mdl.php */