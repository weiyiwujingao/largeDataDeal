 ...
 ****************************************************************/
class Shell_behaviornew_manage_mdl extends CI_Model
{
	/* 博客关注博主对应表 */
    ...
    /**
     * 查用户基本信息的userid用户 用户中心id信息
     * 
     *
     */
    function getAlogUserList($time='') {
        $wheresql = 'userid >0 ';
        //用来获取当天的数据
        if($time)
            $wheresql .= ' and `date` >='.'"'.$time.'"';
        $this->db = $this->load->database('default', true);
        $this->db->query('set names utf8');       
        $res = $this->db->select('userid,data')
