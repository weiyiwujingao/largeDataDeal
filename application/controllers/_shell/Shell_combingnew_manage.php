 ...
 ****************************************************************/
set_time_limit(0);
class Shell_combingnew_manage extends MY_Controller
{
	private $data = array();
   ...
   /**
	...
	{       
        ...
        $this->load->model('Shell_combingnew_manage_mdl');
        $time = date('Y-m-d');//更新当天有更新的用户信息
        $data = $this->Shell_combingnew_manage_mdl->getBehaviorUserid($where,$time);
        if(!$data)exit('已完成更新！');
        foreach($data as $v){
            if(!$v['userid'])continue;
            $behaviordata = unserialize($v['data']); 
            $behaviordata['userid'] = $v['userid'];
            $behaviordata['date']   = date('Y-m-d H:i:s');
            if(isset($behaviordata['author']))
                $behaviordata['author'] = $this->getMaxkeynum($behaviordata['author']);
            if(isset($behaviordata['source']))
                $behaviordata['source'] = $this->getMaxkeynum($behaviordata['source']);
            if(isset($behaviordata['channel']))
                $behaviordata['channel'] = $this->getMaxkeynum($behaviordata['channel']);
            if(isset($behaviordata['keyword']))
                $behaviordata['keyword'] = $this->getMaxkeynum($behaviordata['keyword']);
            if(isset($behaviordata['codename']))
                $behaviordata['codename'] = $this->getMaxkeynum($behaviordata['codename']);
           ...
           // var_dump($behaviordata);exit;
            $this->Shell_combingnew_manage_mdl->insertBehaviorUseridData($behaviordata);
        }
	...
	{       
        ...
        $where = array('offset'=>$offset,'limit'=>$limit);
        $this->load->model('Shell_combingnew_manage_mdl');
        $data = $this->Shell_combingnew_manage_mdl->getBehaviorUserid2($where);
        t($data);
        if(!$data)exit('已完成更新！');
        foreach($data as $v){
     ...
     * @return string
     */
    private function getMaxkeynum($data,$num=10) {
        if(!$data || !is_array($data)) return '';
        $str = '';
        arsort($data);
    ...
    private function insertuser($data) { 
        if(!$data)return;
        $this->load->model('Shell_combingnew_manage_mdl');
        $this->Shell_combingnew_manage_mdl->insertBehaviorUserid($data);        
    }
}
